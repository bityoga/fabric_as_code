...
if (($IDX == 0)); then    
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$CORE_PEER_TLS_ROOTCERT_FILE CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH peer channel create -c appchannel -f /root/${AGENT_HOST}_cli/artifacts/appchannel.tx -o ${ORDERER_HOST}:7050 --outputBlock /root/${AGENT_HOST}_cli/artifacts/appchannel.block --tls --cafile $CORE_PEER_TLS_ROOTCERT_FILE --clientauth --certfile /root/${AGENT_HOST}/msp/tls/server.crt --keyfile /root/${AGENT_HOST}/msp/tls/server.key  || EXIT_CODE=$?
