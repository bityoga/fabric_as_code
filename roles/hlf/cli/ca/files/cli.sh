...
if [$IDX == 0]; then
  ...
  echo "Enroll CA root for $FABRIC_CA_NAME"
  fabric-ca-client enroll -d -u https://ca-admin-$FABRIC_CA_NAME:$FABRIC_CA_SECRET@$FABRIC_CA_NAME:$FABRIC_CA_PORT
    
  echo "Register admin ($ADMIN_USER) at $FABRIC_CA_NAME"
  fabric-ca-client register -d --id.name $ADMIN_USER --id.secret $ADMIN_SECRET --id.type admin --id.attrs "hf.Registrar.Roles=client,hf.Registrar.Attributes=*,hf.Revoker=true,hf.GenCRL=true,admin=true:ecert,abac.init=true:ecert" -u https://$FABRIC_CA_NAME:$FABRIC_CA_PORT

  echo "Enroll admin ($ADMIN_USER) for $FABRIC_CA_NAME"
...
if [ $type == $tlsca ]; then      
  ...
  export FABRIC_CA_CLIENT_HOME=$HOST_HOME/$ADMIN_USER
  echo "Register agent $AGENT_HOST of type $AGENT_TYPE at $FABRIC_CA_NAME"
  ...
  fabric-ca-client enroll -d -u https://$AGENT_HOST:$AGENT_SECRET@$FABRIC_CA_NAME:$FABRIC_CA_PORT --enrollment.profile tls --csr.hosts $AGENT_HOST,127.0.0.1
