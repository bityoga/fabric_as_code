...
binaryIncrementalDownload() {
    ...
    if [ -z "$rc" ] || [ $rc -eq 33 ] || [ $rc -eq 2 ]; then
        ...
        echo "Failure downloading binaries (wget RC=$rc). Please try again and the download will resume from where it stopped."
        exit 1
    fi
}
